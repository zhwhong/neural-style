...
if __name__ == "__main__":
    from data_processing import preprocess_content_image, preprocess_style_image
    from data_processing import postprocess_image, save_image
    vgg_params = mx.nd.load("./model/vgg19.params")
    style_weight = 2
    content_weight = 10
    long_edge = 384
    content_np = preprocess_content_image("./input/IMG_4343.jpg", long_edge)
    style_np = preprocess_style_image("./input/starry_night.jpg", shape=content_np.shape)
    ...
    for e in range(800):
        ...
        if (e+1) % 50 == 0:
            save_image(new_img.asnumpy(), 'output/tmp_' + str(e + 1) + '.jpg')

    save_image(new_img.asnumpy(), "./output/out.jpg")
